...
function ViewSyncRelay( io, config ) {
  var viewState = {
    // start with the default pvd
    pvd: config.provider-1,
    // initialize a google.maps.StreetViewPov
    pov: getInitPov(),
    // start at the default pano
    pano: config.display.default_pano[config.provider-1],
  //  panopvd: {pano:config.display.default_pano[config.provider-1],pvd:config.provider-1}
  };
  ...
  var viewsync = io
    ...
    .on('connection', function (socket) {
      ...
      socket.on('initParam',function(){
        sendParam();
      });
      
       socket.on('panopvd',function(panopvd){
        viewState.pvd = panopvd.pvd;
        viewState.pano = panopvd.pano;
      //  syncBroad( socket,'pvd', panopvd.pvd);
        syncBroad( socket,'panopvd', panopvd);
      }); 
  ...
  return {
    ...
    sendParam: function(){
      var panopvd = {pvd:viewState.pvd, pano:viewState.pano};
      syncSingle(socket,'params',panopvd);
    },
