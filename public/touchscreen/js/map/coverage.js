...
function(config, L, Stapes,  XMaps) {
  ...
  var SVCoverageModule = Stapes.subclass({
    ...
    //  this.XMaps = MMaps[provider];
      ...
      XMaps[provider].addListener(this.map, 'zoom_changed', function(event) {
      ...
      XMaps[provider].addListenerOnce(this.map, 'idle', function() {
        // trigger a zoom change
         XMaps[provider].trigger(self.map, 'zoom_changed');
      });
    },
